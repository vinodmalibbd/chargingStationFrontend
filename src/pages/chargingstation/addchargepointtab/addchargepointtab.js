...
//   span03.innerHTML = `<strong>Status: <p style="color: ${
//     item.available ? "green" : "red"
//   }">${item.available ? "available" : "maintenance"}</p></strong>`;
...
//     if (
//       optionText.toLowerCase().replace(" ", "-") === "available"
//         ? "available"
//         : "maintenance"
//     ) {
...
function editChargingslot(card, item) {
  ...
  statusOptions.forEach((optionText) => {
    ...
    option.textContent = optionText;
    if (
      optionText.toLowerCase().replace(" ", "-") ===
      (item.available ? "available" : "maintenance")
    ) {
      option.selected = true;
    }
    statusSelect.appendChild(option);
...
function showWarningMessage(message) {
  ...
  const warningForAddingSlot = document.querySelector(
    ".chargingstationtabcontainer"
  );
  warningForAddingSlot.appendChild(warningCard);
