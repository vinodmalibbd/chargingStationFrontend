function Rendermap() {
  ...
  getCurrentPositionUser().then((userLocation) => {
    ...
    getAllChargingStation().then((data) => {
      data.forEach((item) => {
        const chargingMarker = L.marker([item.latitude, item.longitude]).addTo(
          map
        );
        chargingMarker.on("click", () => {
          ...
          routingControl = L.Routing.control({
            ...
            .on("routesfound", function (e) {
              ...
              const destinationMarker = L.marker(route.coordinates[0]).addTo(
                map
              );
              ...
              const poupcard = document.querySelector(
